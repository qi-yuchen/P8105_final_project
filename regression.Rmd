...
model_choose = rbind(
 ...
 model_choose %>% 
  select(model_name, adj.r.squared) %>% 
  mutate(model_name = forcats::fct_relevel(model_name, c("lm1", "lm2","lm3","lm4","lm5","lm6","lm7","lm8","lm9","lm10"))) %>% 
  ggplot(aes(x = model_name, y = adj.r.squared)) + 
  geom_point()

rplot

aicplot = ggplot(model_choose, aes(x = model_name, y = AIC)) + 
  geom_point()

bicplot = ggplot(model_choose, aes(x = model_name, y = BIC)) + 
  geom_point()
...
par(mfrow = c(2,2))
plot(lm7)
>>>>>>> e57341d669d2530c99bae5ca4dd84217c208ea46
