...
A further analze in in the 9-year period was applied to Americas, Europe, Asia and Africa except for Oceania which only contained two countries and the average level of continent was nicely representive of the countries, we select several countries in the continents that were relelativly stable in happiness levels, where for America, Europe and Asia use 0.03 as boundary while 0.1 for Afria due to its high variance of happiness level from 2010 to 2018. 
...
Am_happy=
  df %>%
  filter(
    country_name %in% Am_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,life_ladder) %>%
  ggplot(aes(x=year, y=life_ladder,color=country_name))+
  geom_point()+
  geom_line(aes(group=country_name))+labs(y="life ladder")
Am_gdp = 
  df %>%
  filter(
    country_name %in% Am_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,gdp) %>%
  ggplot(aes(x=year, y=gdp,color=country_name))+
  geom_point()+
...
Am_geo = 
  ...
  filter(
    country_name %in% Am_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,generosity) %>%
  ggplot(aes(x=year, y=generosity,color=country_name))+
  geom_point()+
  geom_line(aes(group=country_name))
...
Eu_happy=
  df %>%
  filter(
    country_name %in% Eu_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,life_ladder) %>%
  ggplot(aes(x=year, y=life_ladder,color=country_name))+
  geom_point()+
...
As_happy=
  df %>%
  filter(
    country_name %in% As_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,life_ladder) %>%
  ggplot(aes(x=year, y=life_ladder,color=country_name))+
  geom_point()+
...
Af_happy=
  df %>%
  filter(
    country_name %in% Af_cont$country_name,
    year %in% c(2010:2018)) %>%
  dplyr::select(country_name,year,life_ladder) %>%
  ggplot(aes(x=year, y=life_ladder,color=country_name))+
  geom_point()+
...
```{r}
Am_happy +Am_gdp/Am_geo
Eu_happy +Eu_gdp/Eu_geo
As_happy +As_gdp/As_geo
Af_happy +Af_gdp/Af_geo
